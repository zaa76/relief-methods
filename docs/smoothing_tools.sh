...
make -j 6	
cp apps/gdal_contour <root build relief>/tools/gdal_contour_smooth
popd
...
cargo build --release
cp target/release whitebox_tools <root build relief>/tools
...
cp mdenoise <root build relief>/tools
